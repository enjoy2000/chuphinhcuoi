...
/*
Template Name: Concept Card Stack 7 Images
*/
...
<main class="concept-14 codrops full-height" style="margin: <?php if(!empty($foto_content_padding_top)) { echo sanitize_text_field( $foto_content_padding_top ); ?>px <?php } else { echo "0px "; } ?>0px <?php if(!empty($foto_content_padding_bottom)) { echo sanitize_text_field( $foto_content_padding_bottom ); ?>px<?php } else { echo "0px"; } ?>;">
	<div class="slideshow container" tabindex="0">
			<?php $foto_album_post_list = get_field('album_post_list', false, false);
			$foto_concept_14_args = array(
			...
			$foto_concept_14_loop = new WP_Query($foto_concept_14_args);
			if ($foto_concept_14_loop->have_posts()) : while($foto_concept_14_loop->have_posts()) : $foto_concept_14_loop->the_post();
			...
			$foto_trimmed = wp_trim_words( $foto_gallery_subtitle, $num_words = 20, $more = null ); ?>
				...
				<div class="slide slide--layout-3" data-layout="layout3">
					<div class="slide-imgwrap">
						...
						foreach( array_slice($foto_gallery_images, 0, 7) as $foto_image ): ?>
							
								<div class="slide__img"><div class="slide__img-inner" style="background-image:url(<?php echo esc_url( $foto_image['url'] ); ?>);"></div></div>
