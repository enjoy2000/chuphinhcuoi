...
/*
Template Name: Concept Round Images
*/
...
<main class="concept-17 codrops full-height" style="margin: <?php if(!empty($foto_content_padding_top)) { echo sanitize_text_field( $foto_content_padding_top ); ?>px <?php } else { echo "0px "; } ?>0px <?php if(!empty($foto_content_padding_bottom)) { echo sanitize_text_field( $foto_content_padding_bottom ); ?>px<?php } else { echo "0px"; } ?>;">
	<div class="slideshow container" tabindex="0">
			<?php $foto_album_post_list = get_field('album_post_list', false, false);
			$foto_concept_17_args = array(
			...
			$foto_concept_17_loop = new WP_Query($foto_concept_17_args);
			if ($foto_concept_17_loop->have_posts()) : while($foto_concept_17_loop->have_posts()) : $foto_concept_17_loop->the_post();
			...
			$foto_trimmed = wp_trim_words( $foto_gallery_subtitle, $num_words = 20, $more = null ); ?>
				...
				<div class="slide slide--layout-6" data-layout="layout6">
					<div class="slide-imgwrap">
						...
						$image 		= aq_resize($foto_image['url'],  600, 600, true); 
							?>
