...
/*
Template Name: Concept Album Grid
*/
...
<div class="concept-6 concept-grid-iso content-wrapper <?php if($foto_with_space == true) { ?>with-space<?php } ?> clearfix" style="margin: <?php if(!empty($foto_content_padding_top)) { echo sanitize_text_field( $foto_content_padding_top ); ?>px <?php } else { echo "0px "; } ?>0px <?php if(!empty($foto_content_padding_bottom)) { echo sanitize_text_field( $foto_content_padding_bottom ); ?>px<?php } else { echo "0px"; } ?>;">

	<?php foto_concept_layout_acf_top(); ?>

			<!-- GALLERY START
			============================================= -->
			<div id="iso-grid" class="gallery-section clearfix">
				<?php $foto_album_post_list = get_field('album_post_list', false, false);
				$foto_concept_6_args = array(
				'post_type'			=> 'foto-gallery',
        		'post__in'          => $foto_album_post_list,
				);

				$foto_concept_6_loop = new WP_Query($foto_concept_6_args);
				if ($foto_concept_6_loop->have_posts()) : while($foto_concept_6_loop->have_posts()) : $foto_concept_6_loop->the_post();
				 ?>
