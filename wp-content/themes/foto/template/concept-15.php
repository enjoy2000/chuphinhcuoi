...
/*
Template Name: Concept Asymmetrical 4 Images
*/
...
<main class="concept-15 codrops full-height" style="margin: <?php if(!empty($foto_content_padding_top)) { echo sanitize_text_field( $foto_content_padding_top ); ?>px <?php } else { echo "0px "; } ?>0px <?php if(!empty($foto_content_padding_bottom)) { echo sanitize_text_field( $foto_content_padding_bottom ); ?>px<?php } else { echo "0px"; } ?>;">
	<div class="slideshow container" tabindex="0">
			<?php 
			$foto_album_post_list = get_field('album_post_list', false, false);
			$foto_concept_15_args = array(
			'post_type'			=> 'foto-gallery',
			'post_status'        => 'publish',
			'post__in'			=> $foto_album_post_list,
			);

			$foto_concept_15_loop = new WP_Query($foto_concept_15_args);
			if ($foto_concept_15_loop->have_posts()) : while($foto_concept_15_loop->have_posts()) : $foto_concept_15_loop->the_post();
			...
			$foto_trimmed = wp_trim_words( $foto_gallery_subtitle, $num_words = 20, $more = null ); ?>
				...
				<div class="slide slide--layout-4" data-layout="layout4">
					<div class="slide-imgwrap">
						...
						foreach( array_slice($foto_gallery_images, 0, 4) as $foto_image ): 
							$image 		= aq_resize($foto_image['url'],  1000, 600, true);
							?>
