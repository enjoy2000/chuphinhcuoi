...
/*
Template Name: Concept Rhomboid Images
*/
...
<main class="codrops full-height" style="margin: <?php if(!empty($foto_content_padding_top)) { echo sanitize_text_field( $foto_content_padding_top ); ?>px <?php } else { echo "0px "; } ?>0px <?php if(!empty($foto_content_padding_bottom)) { echo sanitize_text_field( $foto_content_padding_bottom ); ?>px<?php } else { echo "0px"; } ?>;">
	<div class="slideshow container" tabindex="0">
			<?php $foto_album_post_list = get_field('album_post_list', false, false);
			$foto_concept_18_args = array(
			...
			$foto_concept_18_loop = new WP_Query($foto_concept_18_args);
			if ($foto_concept_18_loop->have_posts()) : while($foto_concept_18_loop->have_posts()) : $foto_concept_18_loop->the_post();
